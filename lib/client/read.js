...
function hasObserve(element) {
  return element.name === 'Observe';
}
...
function endRead(req, res) {
  return function (error, result) {
    ...
    if (req.options.some(hasObserve)) {
...
function discoverResourceAttributes(objectUri, resourceId, callback) {
  objectRegistry.getAttributes(objectUri + '/' + resourceId, function (error, attributes) {
    if (error) {
      ...
      if (attributes) {
        payload = Object.entries(attributes)
          .reduce(addAttribute, payload);
      }
...
function discoverObjectResources(objectUri, callback) {
  async.series([
    ...
    var attributes = results[1];

    if (error) {
      callback(error);
    } else {
      var payload = '<' + objectUri + '>',
          resources = Object.keys(attributes);
...
function handleRead(req, res, handler) {
  ...
  function acceptLinkFormat(option) {
    return option.name === 'Accept' && 
      option.value === 'application/link-format';
  }
  ...
  extractUriInfo(req, res, function (error, objectUri, resourceId) {
    if (error) {
      ...
      if (req.options.some(acceptLinkFormat)) {
...
function listObservers(callback) {
  callback(null, Object.values(observers));
}

function cancelAllObservers(callback) {
  var observerValues = Object.values(observers),
      cancellations = [];

  for (var i = 0; i < observerValues.length; i++) {
    cancellations.push(apply(cancelObservation, observerValues[i].uri, 
      observerValues[i].resourceId));
  }
