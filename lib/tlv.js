  ...
  function append(buf, offset, tlv) {
    var types = {
      ...
      Number: function() {
        ...
        case 8:
          // 64-bit
          buf.writeUInt32BE(value >> 32, offset);
          buf.writeUInt32BE(value & 0xffffffff, offset + 4);
          break;
        case 4:
          // 32-bit
      ...
      Date: function() {
        ...
        var timestamp = tlv.value.getTime() / 1e3 >> 0; 

        return append(buf, offset, Object.assign({}, tlv, { 
          value: timestamp,
        }));
      },
    };
...
function parse(payload, schema) {
  function readNumber(pos, len) {
    var value;

    switch (len) {
    case 8:
      // FIXME 64-bit not handled yet.
      value = payload.readUInt32BE(pos + 4);
      break;
    case 4:
      // 32-bit
  ...
  function append(obj, key, type, payload, pos, len) {
    var types = {
      ...
      Date: function() {
        var timestamp = readNumber(pos, len);
  ...
  while (pos < payload.length) {
    ...
    pos = append(result, key, type.name || type, payload, pos, tlv.len);
  }
