...
var libcoap = require('coap'),
    ...
    defaultFormats = require('./contentFormats'),
...
function loadLightweightM2MFormats(config) {
    ...
    if (defaultFormats && defaultFormats.formats) {
...
function startCoap(config, callback) {
    ...
    if (config._udpWindow) {
        serverInfo.window = new Window(config._udpWindow);
    } else {
        serverInfo.window = new Window(100);
    }

    debug('Starting COAP Server on port [%d]', config._port);

    loadLightweightM2MFormats(config);

    serverInfo.server = libcoap.createServer({
        type: config._serverProtocol,
        proxy: true
    });
    ...
    serverInfo.server.listen(config._port, function (error) {
        if (error) {
