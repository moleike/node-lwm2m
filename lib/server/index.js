...
var coapRouter = require('../coapRouter'),
    ...
    registry,
    deviceManagement = require('./deviceManagement'),
...
function loadDefaultHandlers(serverInfo, config) {
    ...
    serverInfo.handlers = {
        registration: {
            module: require('./registration'),
            user: coapRouter.defaultHandler
        },
        unregistration: {
            module: require('./unregistration'),
            user: coapRouter.defaultHandler
        },
        updateRegistration: {
            module: require('./updateRegistration'),
            user: coapRouter.defaultHandler
        }
    };
...
function start(serverConfig, startCallback) {
    ...
    if (config.deviceRegistry && config.deviceRegistry.type === 'mongodb') {
        ...
        registry = require('./mongodbDeviceRegistry');
    } else {
