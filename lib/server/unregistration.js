...
function end(req, res) {
  return function (error, result) {
    if (error) {
      debug('Unregistration request ended up in error `%s`', error.message);
      res.code = error.code || '4.00';
 ...
 */
function applyHandler(handler, client, callback) {
  debug('De-registering device `%s`', client.name);

  handler(client.path + '/' + client.location, callback);
}
