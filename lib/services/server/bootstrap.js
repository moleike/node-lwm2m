...
var async = require('async'),
    ...
    senml = require('../shared/senml'),
    tlv = require('../shared/tlv'),
...
function _write(device, path, payload, mediaType, callback) {
    function createRequest(callback) {
        var request = {
            host: (config.ipProtocol === 'udp6')?'::1':'127.0.0.1',
            port: config.port,
            method: 'PUT',
            proxyUri: 'coap://' + (config.ipProtocol === 'udp6' ? '['+device.address+']' : device.address) + ':' + device.port,
            pathname: path,
            payload: payload,
            options: {
...
function getPath(path) {
    var uriParts = path.split('/').filter(Number).map(Number);
    
    if ([1,2,3].indexOf(uriParts.length) < 0) {
      throw new Error('Bad LWM2M URI `' + path + '`');
    }

    return {
      objectId:   uriParts[0],
      instanceId: uriParts[1],
      resourceId: uriParts[2]
    };
}
...
function write(device, path, value, options, callback) {
    ...
    try { uri = getPath(path); } catch (e) {
        callback(e);
    }

    var mediaType = opt.mediaType || config.writeFormat;

    if (opt.schema && opt.mediaType) {
        var obj = {};

        if (uri.resourceId) {
            obj[uri.resourceId] = value;
        } else {
            obj = value;
        }
        ...
        if (mediaType.match(/json/)) {
          ...
          serialize = senml.serialize;
        } else if (mediaType.match(/tlv/)) {
          mediaType = 'application-vnd-oma-lwm2m/tlv';
          serialize = tlv.serialize;
        }

        try {
          payload = serialize(obj, opt.schema);
        } catch (e) {
          callback(e);
        }
    }

    _write(device, path, payload, mediaType, callback);
}
